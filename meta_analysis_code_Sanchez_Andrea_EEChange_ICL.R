...
plot(x=hat.abun.overall$hat.abun.overall, y= rs.abun.overall$resid, 
     ylab= "Standardized residuals", xlab= "Hat values", main= "A) Database: Abundance. Intercept only model.", cex.main=0.9)
...
plot(x=hat.abun.FG$hat.abun.FG, y= rs.abun.FG$resid, 
     ylab= "Standardized residuals", xlab= "Hat values", main= "B) Database: Abundance. Meta-regression model (Moderator = Functional groups)", cex.main=0.8)
...
abun.tabletext<-abun.comb%>%
  ...
  add_row(FG= c("Functional groups"),ES_LRR = c("ES"), SE = c("SE"), tval= c("t-value"), 
          pval= c("p-value"), ci.lb= c("LC"), ci.ub= c("UC"), n_studies= c("# Studies"), 
...
plot(x=hat.abun.natural_percentage$hat.abun.natural_percentage, y= rs.abun.natural_percentage$resid, 
     ylab= "Standardized residuals", xlab= "Hat values", main= "C) Database: Abundance. Meta-regression model (Moderator = % natural habitats)", cex.main = 0.9)
...
plot(x=hat.abun.arable_percentage$hat.abun.arable_percentage, y= rs.abun.arable_percentage$resid, 
     ylab= "Standardized residuals", xlab= "Hat values", main= "D) Database: Abundance. Meta-regression model (Moderator = % agricultural areas)", cex.main = 0.8)
...
plot(abundance_logRR$min_log_distance_mean, rs.abun.log_distance$resid,
     ylab="Standardized residuals", xlab= "ln(Distance + 1)", main = "B", cex.main=0.7)
lines(x=preds.abun.log_distance$X.min_log_distance_mean ,y=preds.abun.log_distance$pred, col="#686D35", lwd=2)
mtext("Database: Abundance", outer = TRUE, cex = 2)
...
plot(x=hat.abun.log_distance$hat.abun.log_distance, y= rs.abun.log_distance$resid, 
     ylab= "Standardized residuals", xlab= "Hat values", main= "E) Database: Abundance. Meta-regression model (Moderator = distance to natural habitats)", cex.main= 0.8)
...
##----Plot LANDSCAPE ANALYSIS ABUNDANCE
library(cowplot)
#https://rstudio-pubs-static.s3.amazonaws.com/93611_716b9b330cd54c0196916d6c76052bdc.html
preds.abun.arable <-predict(abun.arable_percentage.sensitivity,newmods=c(0,25,50,75,100,105), addx=TRUE)
...
abun.arable.summary<- coef(summary(abun.arable_percentage.sensitivity))%>%
...
figure_3A<- ggplot(preds.abun.arable,aes(x=X.arable_percentage_mean,y=pred))+
  ...
  theme(
    axis.text.x = element_text(color="#22211d",size=14,  family = "sans"),
    axis.text.y = element_text(color="#22211d",size=14, family = "sans"),
    text = element_text(color = "#22211d", size =15, face = "bold", family = "sans"),
    plot.background = element_rect(fill = "White", color = "White"), 
    panel.background = element_rect(fill = "White", color = "White"), 
    axis.line = element_line(colour = "black"))
figure_3A

preds.abun.log_distance_plot <-predict(abun.log_distance,newmods=c(0:8), addx=TRUE)
preds.abun.log_distance_plot<-as.data.frame(preds.abun.log_distance_plot)
preds.abun.log_distance_plot

abun.log_distance.summary<- coef(summary(abun.log_distance))%>%
  mutate(estimate= round(estimate, digits = 4),
         tval= round(tval, digits =2),
         pval= round(pval, digits =2),
         ci.lb = round(ci.lb, digits =5),
...
abun.log_distance.summary

figure_3B<- ggplot(preds.abun.log_distance_plot,aes(x=X.min_log_distance_mean,y=pred))+
  geom_point(data=abundance_logRR,aes(x=min_log_distance_mean,y=LRR),colour="grey70",fill="white",
             position="jitter",alpha=0.4,shape= 20,size=2)+
  stat_smooth(method="lm",formula=y~x,fullrange=T,se=FALSE,size=1.5, colour="#686D35")+
  stat_smooth(data=preds.abun.log_distance_plot,aes(x=X.min_log_distance_mean,y=ci.lb),method="lm",formula=y~x,
              fullrange=T,se=FALSE,size=1,linetype=2,colour="#686D35")+
  stat_smooth(data=preds.abun.log_distance_plot,aes(x=X.min_log_distance_mean,y=ci.ub),method="lm",formula=y~x,
              fullrange=T, se=FALSE,size=1,linetype=2,colour="#686D35")+
  geom_hline(yintercept = 0, colour = "grey50", )+
  scale_y_discrete(limits = c(-8,-6,-4,-2,0,2,4,6))+
  ...
  #geom_label(data=abun.log_distance.summary, mapping= aes(x=x, y=y,label= label),
            # size=4.5, family = "sans")+
  theme(
    axis.text.x = element_text(color="#22211d",size=14,  family = "sans"),
    axis.text.y = element_text(color="#22211d",size=14, family = "sans"),
...
plot_grid(figure_3A,figure_3B, labels = "AUTO")
...
plot(x=hat.richness.overall$hat.richness.overall, y= rs.richness.overall$resid, 
     ylab= "Standardized residuals", xlab= "Hat values", main = "F) Database: Species richness. Intercept only model", cex.main=0.8)
...
plot(x=hat.richness.FG$hat.richness.FG, y= rs.richness.FG$resid, 
     ylab= "Standardized residuals", xlab= "Hat values", main = "G) Database: Species richness. Meta-regression model (Moderator = functional groups)", cex.main=0.8)
...
##Figure: Forest plot 
appendix_H13<- forestplot(rich.tabletext_sensitivity, mean= rich.comb.graph_sensitivity$ES_percent, lower= rich.comb.graph_sensitivity$ci.lb_percent, 
...
plot(x=hat.richness.natural_percentage$hat.richness.natural_percentage, y= rs.richness.natural_percentage$resid, 
     ylab= "Standardized residuals", xlab= "Hat values", main = "H) Database: Species richness. Meta-regression model (Moderator = % natural habitats)", cex.main=0.8)
...
plot(x=hat.richness.arable_percentage$hat.richness.arable_percentage, 
     y= rs.richness.arable_percentage$resid, ylab= "Standardized residuals", xlab= "Hat values", main = "I) Database: Species richness. Meta-regression (Moderator = % agriculturlal areas)", cex.main=0.8)
...
plot(x=hat.richness.log_distance$hat.richness.log_distance, 
     y= rs.richness.log_distance$resid, ylab= "Standardized residuals", xlab= "Hat values", main = "J) Database: Species richness. Meta-regression (Moderator = distance to natural habitats)", cex.main= 0.8)
...
preds.rich.log.distance_plot <-predict(richness.log_distance,newmods=c(0:8), addx=TRUE)
preds.rich.log.distance_plot<-as.data.frame(preds.rich.log.distance_plot)
class(preds.rich.log.distance_plot)
preds.rich.log.distance_plot
...
figure_5<- ggplot(preds.rich.log.distance_plot,aes(x=X.min_log_distance_mean,y=pred))+
  ...
  stat_smooth(data=preds.rich.log.distance_plot,aes(x=X.min_log_distance_mean,y=ci.lb),method="lm",formula=y~x,
              fullrange=T,se=FALSE,size=1,linetype=2,colour="#A63117")+
  stat_smooth(data=preds.rich.log.distance_plot,aes(x=X.min_log_distance_mean,y=ci.ub),method="lm",formula=y~x,
              fullrange=T, se=FALSE,size=1,linetype=2,colour="#A63117")+
  geom_hline(yintercept = 0, colour = "grey50" )+
  scale_x_discrete(limits=c(0, 2, 4, 6, 7, 8),labels=c("0", "2", "4", "6",">7", ""))+
  labs(x=expression(bold(paste("Distance to natural or semi-natural habitats [ln(distance + 1)]"))),
